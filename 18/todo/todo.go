...
func Add(list List, item Item) (List, Item, error) {
	_, err := ParseStatus(item.Status)
	if err != nil {
		return list, item, fmt.Errorf("Invalid status value: %s", item.Status)
	}
...
func Update(list List, item Item) (List, error) {
	_, err := ParseStatus(item.Status)
	if err != nil {
		return list, fmt.Errorf("Invalid status value: %s", item.Status)
	}
